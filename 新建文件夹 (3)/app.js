// 初始化 LeanCloud
AV.init({
    appId: process.env.LEANCLOUD_APP_ID || "H0RYZUGpZ6auG7Njrd4fbyWD-gzGzoHsz",
    appKey: process.env.LEANCLOUD_APP_KEY || "tenY3Ksbmgns38v74l48Symx",
    serverURL: process.env.LEANCLOUD_SERVER_URL || "https://h0ryzugp.lc-cn-n1-shared.com",
    production: true,
    headers: {
...
async function uploadFile() {
    ...
    try {
        ...
        if (error.code === 403) {
            errorMessage += '权限被拒绝，请检查域名配置';
        } else if (error.code === 404) {
            errorMessage += '服务未找到，请检查服务器配置';
        } else {
...
async function loadFiles() {
    try {
        ...
        const filesList = document.getElementById('filesList');
        filesList.innerHTML = `<li style="color: red;">${errorMessage}</li>`;
    }
}
